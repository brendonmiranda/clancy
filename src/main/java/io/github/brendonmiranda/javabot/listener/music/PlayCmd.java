...
public class PlayCmd extends Command {
    ...
    protected void execute(final CommandEvent event) {
        ...
        event.getGuild().getAudioManager().openAudioConnection(event.getGuild().getVoiceChannelsByName("Geral",true).get(0));
        event.getGuild().getAudioManager().setSendingHandler(new AudioSendHandlerImpl(audioPlayer));
        audioPlayerManager.loadItem("https://www.youtube.com/watch?v=EBLF26-Irdc", new AudioLoadResultHandlerImpl(audioPlayer));
    }
